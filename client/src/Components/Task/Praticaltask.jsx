...
const Praticaltask = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:3001/pratical")
    ...
    const submit = (e) => {
        e.preventDefault();
        axios.post("http://localhost:3001/pratical", { content })
            .then(result => {
                console.log(result);
                navigate('/pratical');
                toggleSearch();
    ...
    const handleDelete = (taskId) => {
        axios.delete(`http://localhost:3001/pratical/${taskId}`)
            .then(result => {
                console.log(result);
